...
class TestSLRUAccuracy(unittest.TestCase):
    ...
    def test_eviction_mechanism(self):
        ...
        frequent_key = RadixKey(token_ids=[1, 2], extra_key=None)  # High hit rate, should be retained
        ...
        first_low_freq_key = RadixKey(token_ids=[5, 6], extra_key=None)  # Low hit rate, should be evicted
        first_low_freq_val = torch.tensor([50, 60], dtype=torch.int64)

        self.cache.insert(InsertParams(key=first_low_freq_key, value=first_low_freq_val))
        ...
        for i in range(4):  # Reduce the number to fit in our smaller cache
            key = RadixKey(token_ids=[i+10], extra_key=None)  # Unique keys for low-frequency items
        ...
        for i in range(6, 10):  # Add more items to definitely exceed capacity
            key = RadixKey(token_ids=[i*2], extra_key=None)  # Different pattern to avoid conflicts
            val = torch.tensor([i*200], dtype=torch.int64)
            self.cache.insert(InsertParams(key=key, value=val))

        # Now trigger eviction explicitly to make space
        evict_result = self.cache.evict(EvictParams(num_tokens=4))  # Try to evict 4 tokens worth of space
        ...
        frequent_match_result = self.cache.match_prefix(MatchPrefixParams(key=frequent_key))
        ...
        first_low_freq_match_result = self.cache.match_prefix(MatchPrefixParams(key=first_low_freq_key))

        # Verify the frequent key is still present in cache after evictions
        self.assertIsNotNone(frequent_match_result, "Frequently accessed key should still be in cache after evictions")
        ...
        self.assertTrue(is_frequent_key_present, "Frequently accessed key should still be in cache after evictions")

        # Verify the first low-frequency key has been evicted
        # The device_indices tensor should be empty when the key is not found
        is_first_low_freq_key_present = first_low_freq_match_result.device_indices.numel() > 0
        self.assertFalse(is_first_low_freq_key_present, "First inserted low-frequency key should be evicted after evictions")
